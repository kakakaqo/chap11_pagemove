...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String errorMsg = "";
		// doPost메소드에서 쿼리스트링에 달아보낸 errorMsg추출
		if(request.getParameter("errorMsg") !=null) {
			
			/*
			 * [디코딩]
			 * 오류 메시지 보내는 쪽에서 utf-8 방식으로 2byte씩 인코딩 했으므로
			 * 받는쪽에서도 utf-8방식으로 2byte씩 디코딩해야 완벽한 메시지복원됨.
			 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		request.setCharacterEncoding("utf-8");
		// 사용자 브러우저에 내려보내는 컨텐트 타입 설정
		response.setContentType("text/html; charset=utf-8");
		// 사용자 브러우저에 응답할 때 한글이 깨지지 않고 전달되도록 설정
		...
		if(isAuthenticated) {
			...
			/*
			 * [한글 메시지의 경우 인코딩]
			 * - 한글 메세지를 Url에 쿼리스트링 형태로 달고 가기 위해 인코딩
			 * - Url에 달아서 보낼 메세지 인코딩(UTF-8)해서 2byte씩 처리
			 * - 웹페이지의 본문(메세지 바디) 에서는 UTF-8을 허용하지만,
			 *   웹브라우저의 주소창에서는 오로지 ASCII 코드만 하용해서 문제가됨.
			...
			/*
			 * sendRedirect는 get 방식 요청으로 주소창에 파라미터를 달고감.
